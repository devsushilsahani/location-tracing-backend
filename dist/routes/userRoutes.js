...
router.get('/location/:userId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const latestLocation = yield prisma.location.findFirst({
            where: {
                route: {
                    userId: userId
                }
...
router.get('/device-location/:deviceId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
router.get('/latest-locations', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { limit = '10' } = req.query;
        const limitNum = parseInt(String(limit), 10);
        // Get the most recent locations
        const latestLocations = yield prisma.location.findMany({
            orderBy: {
                timestamp: 'desc'
            },
            take: limitNum,
...
router.post('/trace-movement', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (userId) {
            ...
            let activeRoute = yield prisma.route.findFirst({
                where: {
                    userId: userId,
                    endTime: {
            ...
            if (!activeRoute) {
                try {
                    activeRoute = yield prisma.route.create({
                        data: {
                            userId: userId,
                            date: new Date(),
        ...
        if (routeId) {
            const previousLocation = yield prisma.location.findFirst({
                where: {
                    routeId: routeId,
                    id: { not: location.id }
                },
            ...
            if (previousLocation) {
                ...
                const distance = calculateDistance(previousLocation.latitude, previousLocation.longitude, latitude, longitude);
                // Update the route with the new distance
                yield prisma.route.update({
                    where: { id: routeId },
                    data: {
...
router.post('/complete-route/:userId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { userId } = req.params;
        if (!userId) {
            return res.status(400).json({ error: 'userId is required' });
        ...
        const activeRoute = yield prisma.route.findFirst({
            where: {
                userId: userId,
                endTime: {
                    gte: new Date('2099-01-01T00:00:00Z') // Check for our "unfinished" route marker
        ...
        const completedRoute = yield prisma.route.update({
...
router.get('/return-to-start/:routeId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const currentPoint = yield prisma.location.findFirst({
            where: {
                routeId: routeId
            },
            orderBy: {
        ...
        const distanceToStart = calculateDistance(currentPoint.latitude, currentPoint.longitude, startingPoint.latitude, startingPoint.longitude);
        // Calculate bearing to starting point
        const bearingToStart = calculateBearing(currentPoint.latitude, currentPoint.longitude, startingPoint.latitude, startingPoint.longitude);
        return res.json({
            startingPoint,
            currentPoint,
            navigationInfo: {
...
router.get('/history/:userId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const routes = yield prisma.route.findMany({
            where: Object.assign({ userId: userId, endTime: {
        ...
        const totalRoutes = yield prisma.route.count({
            where: Object.assign({ userId: userId, endTime: {
                    lt: new Date('2099-01-01T00:00:00Z') // Only include routes with real end times
                } }, (Object.keys(dateFilter).length > 0 ? { date: dateFilter } : {}))
        });
...
router.get('/route-details/:routeId', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { routeId } = req.params;
        if (!routeId) {
            return res.status(400).json({ error: 'routeId is required' });
